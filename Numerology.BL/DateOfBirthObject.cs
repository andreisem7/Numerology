...
namespace Numerology.BL
{
    public class DateOfBirthObject
    {
        ...
        public string GetLifeWayNumberFullCalculationString
        {
            get { return lifeWayNumberFullCalculation; }
        }
        ...
        private void InitPeaks()
        {
            ...
            peak1.Peak = int.Parse(NarrowToOneNumber(GetDayString + GetMonthString, out isMasterNumberPeakPlus_1, out masterNumberPeakPlus_1, out _));
            peak1.IsPeakMaster = isMasterNumberPeakPlus_1;
            peak1.PeakMaster = masterNumberPeakPlus_1;
            peak1.Bottom = Math.Abs(int.Parse(NarrowToOneNumber(DOB.Day.ToString(), out _, out _, out _)) -
                int.Parse(NarrowToOneNumber(DOB.Month.ToString(), out _, out _, out _)));
            peak1.StartYear = DOB;
            ...
            peak2.Peak = int.Parse(NarrowToOneNumber(GetDayString + GetYearString, out isMasterNumberPeakPlus_2, out masterNumberPeakPlus_2, out _));
            peak2.IsPeakMaster = isMasterNumberPeakPlus_2;
            peak2.PeakMaster = masterNumberPeakPlus_2;
            peak2.Bottom = Math.Abs(int.Parse(NarrowToOneNumber(DOB.Day.ToString(), out _, out _, out _)) -
                int.Parse(NarrowToOneNumber(DOB.Year.ToString(), out _, out _, out _)));
            ...
            peak3.Peak = int.Parse(NarrowToOneNumber(GetDayString + GetMonthString + GetDayString + GetYearString, out isMasterNumberPeakPlus_3, out masterNumberPeakPlus_3, out _));
            peak3.IsPeakMaster = isMasterNumberPeakPlus_3;
            peak3.PeakMaster = masterNumberPeakPlus_3;
            peak3.Bottom = Math.Abs(Math.Abs(int.Parse(NarrowToOneNumber(DOB.Day.ToString(), out _, out _, out _)) -
                int.Parse(NarrowToOneNumber(DOB.Month.ToString(), out _, out _, out _))) -
                Math.Abs(int.Parse(NarrowToOneNumber(DOB.Day.ToString(), out _, out _, out _)) -
                int.Parse(NarrowToOneNumber(DOB.Year.ToString(), out _, out _, out _))));
            ...
            peak4.Peak = int.Parse(NarrowToOneNumber(GetMonthString + GetYearString, out isMasterNumberPeakPlus_4, out masterNumberPeakPlus_4, out _));
            peak4.IsPeakMaster = isMasterNumberPeakPlus_4;
            peak4.PeakMaster = masterNumberPeakPlus_4;
            peak4.Bottom = Math.Abs(int.Parse(NarrowToOneNumber(DOB.Month.ToString(), out _, out _, out _)) - int.Parse(NarrowToOneNumber(DOB.Year.ToString(), out _, out _, out _)));
        ...
        private int GetLifeWayNumberMethod(string day, string month, string year, out bool isMaster, out int masterNumber, out string lifeWayNumberFullCalculation)
        {
            isMaster = false;
            masterNumber = 0;
        ...
        private string NarrowToOneNumber(string input, out bool isMaster, out int masterNumber, out string lifeWayNumberFullCalculation)
        {
            ...
            while (result.Length > 1)
            {
                ...
                if (!string.IsNullOrEmpty(lifeWayNumberFullCalculation)) lifeWayNumberFullCalculation += delimeter + result;
                else lifeWayNumberFullCalculation = result;
        ...
        public static string GetString(int value, int stringLength)
        {
